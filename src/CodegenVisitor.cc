...
// 生成常数值
Value *ConstAST::codegen(CodegenVisitor &v) {
  if (Type == "int") {
...
// 这个是新建声明
Value *DeclAST::codegen(CodegenVisitor &v) {
  ...
  for (auto &&init_dcl : children[0]->children) {
    ...
    if (init_dcl->children.size() >= 1) {
      // 直接声明符
      std::shared_ptr<DirectDclAST> decl =
          std::dynamic_pointer_cast<DirectDclAST>(init_dcl->children[0]);
      ...
      if (decl->size == 1) {
        ...
        if (v.SymbolTables.size() == 1) {
          ...
          if (init_dcl->children.size() == 2) {
            printf("start init\n");
            default_init =
                dyn_cast<Constant>(init_dcl->children[1]->codegen(v));
        ...
        // local
        // 如果是局部变量 就直接 Create alloca 你记不记得 alloca 1 就是
        // ir 的 alloca 语句
        else {
      ...
      // 数组 内容极其少 其实应该主要是初始化比较难 hao
        ...
        Value *arraySize =
            ConstantInt::get(Type::getInt64Ty(*v.TheContext), decl->size, true);
...
// 函数定义！
Value *FuncDefAST::codegen(CodegenVisitor &v) {
  ...
  Function *func =
      Function::Create(ft, GlobalValue::ExternalLinkage,
                       direct_dcl->id_name.c_str(), *v.TheModule.get());
  printf("- producing function body\n");

  BasicBlock *BBEntry =
      BasicBlock::Create(*v.TheContext, "entry", func, nullptr);
  ...
  if (params != nullptr) {
    ...
    for (auto &&func_args : func->args()) {
      auto direct_dcl =
          (dynamic_pointer_cast<DirectDclAST>((*arglist)->children[0]));
      func_args.setName(direct_dcl->id_name);
      Value *allocv;
      if (direct_dcl->size == 1) {
        allocv = v.Builder->CreateAlloca(v.s2Type((*arglist)->Type));
      } else {
        allocv = v.Builder->CreateAlloca(PointerType::get(
            v.s2Type((*arglist)->Type), 0)); // 这里就是数组传指针了
      }

      v.Builder->CreateStore(&func_args, allocv,
                             false); // 前面的 store 都是这里来的 wo
...
// 一堆语句 这是 {int a; .... ;} 的结点
llvm::Value *StatListAST::codegen(CodegenVisitor &v) {
  printf("\ngenerating block.\n");
  Value *vl;
...
// 返回值
llvm::Value *RetStatAST::codegen(CodegenVisitor &v) {
...
// 一元运算符
llvm::Value *UnaryExprAST::codegen(CodegenVisitor &v) {
  // 求操作数的值
  ...
  if (name == "-") {
    ...
    // ... 所以 int 也要转回指针 hao
    auto vderef =
        v.Builder->CreateIntToPtr(operand, Type::getInt64PtrTy(*v.TheContext));
...
// 二元运算符 很无聊的
llvm::Value *BinaryExprAST::codegen(CodegenVisitor &v) {
  ...
  if (name == "+") {
    ...
    if (oaddr->getType()->getPointerElementType()->isPointerTy()) {
      oaddr = v.Builder->CreateGEP(
          oaddr->getType()->getPointerElementType()->getPointerElementType(),
          v.Builder->CreateLoad(oaddr->getType()->getPointerElementType(),
                                oaddr),
...
// 读取标识符的数值
llvm::Value *ReadAST::codegen(CodegenVisitor &v) {
  printf("read value\n");
  v.outSymbolTable();
  ...
  Value *id_value = v.Builder->CreateLoad(
      id_ptr_value->getType()->getPointerElementType(), id_ptr_value,
      ""); // 刚刚看到的 load 是这里来的 hao
  ...
  if (id_value->getType()->isPointerTy()) {
    Value *array_value =
        v.Builder->CreateGEP(id_value->getType()->getPointerElementType(),
                             id_value, children[1]->codegen(v), "");
    return v.Builder->CreateLoad(id_value->getType()->getPointerElementType(),
                                 array_value);
  }
...
// 表达式语句 很平凡
Value *ExprStatAST::codegen(CodegenVisitor &v) {
  return children[0]->codegen(v);
}

// if else
Value *IfElseStatAST::codegen(CodegenVisitor &v) {
  printf("ifelse stat\n");
  ...
  Function *functx =
      v.Builder->GetInsertBlock()->getParent(); // 获取 现在 在哪个函数里
  ...
  if (thenBB->getTerminator() == nullptr) // 如果 thenBB 里没有 return
                                          // 那还要回到 if 外面 函数里面的地方
  {
...
llvm::Value *LoopStatAST::codegen(CodegenVisitor &v) {
...
Value *ContStatAST::codegen(CodegenVisitor &v) {
  return v.Builder->CreateBr(v.loopHeads.back());
}

Value *BreakStatAST::codegen(CodegenVisitor &v) {
  return v.Builder->CreateBr(v.loopEnds.back());
}

Value *CallExprAST::codegen(CodegenVisitor &v) {
