...
//     return v.retv = llvm::ConstantInt::get(*(v.TheContext), llvm::APInt(32,
//     value));
// }
...
//     llvm::FunctionType::get(llvm::Type::getInt32Ty(*v.TheContext), false);

//     llvm::Function* mainf = llvm::Function::Create(ft,
//         llvm::Function::ExternalLinkage, "jialmain", v.TheModule.get());

//     llvm::BasicBlock *BB = llvm::BasicBlock::Create(*v.TheContext, "main",
//     mainf);
...
void make_function(tnode scope, string type_spec, tnode direct_dcl,
...
void make_decl(tnode scope, string type_spec, tnode init_scl_list) {
...
tnode make_directdcl(string identifier) {
...
tnode make_directdcl_array(tnode direct_dcl, int arrlen) {

  direct_dcl->name += "is a array size " + to_string(arrlen);
  direct_dcl->Type = "array";
  shared_ptr<DirectDclAST> direct_dcl_t =
      dynamic_pointer_cast<DirectDclAST>(direct_dcl);
  direct_dcl_t->size *= arrlen;
  return direct_dcl;
}

tnode make_parameter_decl(string type_spec, tnode directdcl) {
...
tnode make_parameter_list(string type_specifier, tnode directdcl) {
...
tnode cons_parameter_list(tnode cdr, string car_type, tnode car_dcl) {
...
tnode make_initdcl(tnode direct_dcl, tnode init) {
...
tnode make_init(tnode expr) { return expr; }
...
tnode make_ret_stat(tnode expr) {
...
tnode make_stat_list(tnode stat) {
...
tnode make_unary(string op, tnode operand) {
...
tnode make_binary(string op, tnode operand1, tnode operand2) {
